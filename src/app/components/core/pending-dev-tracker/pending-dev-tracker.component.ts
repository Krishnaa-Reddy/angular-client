...
import { lucideChevronsLeft, lucideChevronLeft, lucideChevronRight, lucideChevronsRight, lucideBadgeCheck, lucideClock2, lucideBan } from '@ng-icons/lucide';
...
import { TaskStatus, StatusTemplate, Task } from '../../../../types/dev-tasks.type';
...
export class PendingDevTrackerComponent {
  ...
  protected _tasks: Task[] = [
    ...
    { task: "Optimize the dsa-server.service and routing-service", status: 'TODO' },
    { task: "Use rxjs combinational operators to simply async operations b/w routes & server-data", status: 'TODO' },
  ...
  _tasksUpdated = this._tasks.map(task => <{task:Task, statusTemplate: StatusTemplate}>{task, statusTemplate: this.statusTemplates[task.status]})
